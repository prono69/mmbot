...
start_and_help = InlineKeyboardMarkup(
    [
        [InlineKeyboardButton(text="Creator 🦾", url="https://t.me/MosheNew")],
        [
            InlineKeyboardButton(
                text="Source Code 🗃", url="https://github.com/moshe-coh/Terminal-Bot"
            )
        ],
    ]
)

refresh_space = InlineKeyboardMarkup(
    [[InlineKeyboardButton(text="Refresh 💫", callback_data="space")]]
)
...
def folder_markup():
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                f"⬆️ {os.getcwd().split('/')[-2]}", callback_data=f"folder={'up'}"
            )
        ]
    )
    for folder in directories[current_items[1] : current_items[1] + 10]:
        ...
        keyboard.append(
            ...
            [
                InlineKeyboardButton(
                    f"📁 {folder.name}",
                    callback_data=f"folder={str(directories.index(folder))}",
                )
            ]
        )
    ...
    keyboard.append([InlineKeyboardButton("🗃 Show Files", callback_data="ShowFiles")])
    keyboard.append(
...
def files_markup():
    ...
    for file in files[current_items[0] : current_items[0] + 10]:
        keyboard.append(
            [
                InlineKeyboardButton(
                    f"{'🗃'} {file.full_name}", callback_data=f"file={files.index(file)}"
                )
            ]
        )
    keyboard.append(
...
async def callback(_, cb: CallbackQuery):
    ...
    if data.startswith("ShowFolders"):
        ...
        try:
            os.remove(name)
            await cb.message.edit_text(
                f"File {name} Deleted!", reply_markup=files_markup()
            )
        ...
        try:
            ...
            await cb.message.edit_text(
                f"Renamed successfully", reply_markup=files_markup()
            )
        except TimeoutError:
            pass
        except Exception as e:
        ...
        try:
            ...
            await cb.message.edit_text(
                f"Can't generate file link for your file...\n\n{str(e)}"
            )
        ...
        # Some TV like Honor's Harmony OS need a filename in it, so give a file
        # name 'test.mp4'.
        dlink = (
            f"https://{filebrowser_server}/fb/api/public/dl/{hash}?filename=test.mp4"
        )
        dlink = dlink.replace(" ", "\ ")
        ...
        await cb.message.edit_text(
            f"folder {folder_name} successfully deleted", reply_markup=base_markup
        )
    elif data == "space":
